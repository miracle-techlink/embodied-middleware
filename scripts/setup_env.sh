...
set -e
HERE="$(cd "$(dirname "$0")/.." && pwd)"             # 仓库根(scripts/ 的上一级)
...
# ---- 3) 采集插件三件套 -------------------------------------------------------
export LEROBOT_SRC
bash "$HERE/lerobot_plugins/install.sh"
bash "$HERE/lerobot_plugins/install_orbbec.sh"
bash "$HERE/lerobot_plugins/install_depthfix.sh"
...
[手动] 还需 3 个 sudo 步骤(或重跑 WITH_SUDO=1 bash scripts/setup_env.sh):
  ...
  3) 每次采集前拉起 CAN:sudo bash $HERE/scripts/setup_rebot_can.sh
EOF
fi
